...
function invoiceNeed(data){
	if(data == '1'){
		$("#firstInvoice").css("display","none");
		buttonDeal(1,"beforeInvoice",2);
	}else{
		$("#firstInvoice").css("display","table-row");
	}
}
...
//	}
    ...
    if(discount != ""){
        ...
        if(!reg.test(discount)){
            $("#discount_err").text("折扣率格式不正确，格式为8或者78,80");
            return;
        }else{
            $("#discount_err").text("");
        }
...
function clearOrderAll(){
	$("#invoice0").attr("checked",'true'); 
	$("#invoice1").removeAttr("checked");
...
function changePage2(){
	...
	var total=0;
	for(var i=0;i<orderList.length;i++){
		var sum=parseInt(""+orderList[i].sum,10);
		if(orderList[i].monthChoice == '1'){
			var str=orderList[i].time2.split(",");
			for(var k=0;k<str.length;k++){
				var dateInfo=str[k].split("-");
				//如果是合刊，就把订阅月数加11
				if(dateInfo[1]=='13'){
					sum+=11;
				}
		...
		var money=(orderList[i].productType=='1'?38:50);
		//var discount=$("#discount").val();
		total+=parseInt(""+orderList[i].orderCount,10)*sum*money;
//		if(discount != ""){
//			var discountInfo=parseInt(discount,10)<10?parseInt(discount,10)*10:parseInt(discount,10);
...
function saveInfo(){
	...
	if(updateOrderInfoFlag){
		...
		jQuery.startLoading();
		jQuery.ajax({
		   url: "/ccms1/selectSeq?date="+new Date().getTime(),
...
function addCol(order) {  
    $col = $("<tr><td width='20%'>"+order.OrderSeq+"</td>" +
    		...
    		"<td><a href='#a1' class='edit' onclick='orderInfoDetail(this,\""+order.OrderSeq+"\")'></a>" +
...
function finishSubmit(){
	...
	var discount=$("#discount").val();
	 ...
	 jQuery.ajax({
		   type: "POST",
		   url: "/ccms1/computeMoney?discount="+discount,
		   data: {"orderList":$.toJSON(orderList)},
		   dataType: 'json',
		   success: function(data){
			   jQuery.endLoading();
			   $("#succ_tips_lable1").html("订单总金额为"+data.totalMoney+"元<br/>其中包含邮费"+data.postMoney+"元");
	           $("#succ_tips_layer1").show().centerV();
		   }
	 });
}
function insertOrder(){
	...
	user.textInfo=textInfo;
	 for(var i=0;i<2;i++){
		 if($("#invoice"+i).attr("checked")){
			 user.invoiceDeal=i+"";
		}
	 } 
	 for(var i=0;i<2;i++){
		 if($("#beforeInvoice"+i).attr("checked")){
			 user.beforeInvoice=i+"";
		}
	 } 
	 jQuery.startLoading();
	 jQuery.ajax({
		   type: "POST",
		   url: "/ccms1/saveOrderInfo",
		   ...
		   success: function(data){
			   ...
			   if(data!=null && data.errorMsg!=null){
				   ...
				   window.location.href="/ccms1/comPage?data=saleOrderInfo";
			   }
		   },
		   error: function(data) {
			   jQuery.endLoading();
